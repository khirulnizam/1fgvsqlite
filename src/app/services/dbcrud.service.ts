...
export class DbcrudService {
  ...
  databaseConn() {
    this.platform.ready().then(() => {
      this.sqlite.create({
          ...
          sqLite.executeSql("CREATE TABLE IF NOT EXISTS userTable "+
  ...
  public addItem(n, e) {
    ...
    this.dbInstance.executeSql("INSERT INTO userTable (name, email) VALUES ('"+n+"','"+e+"');", [])
      .then(() => {
        alert("Success");
        this.getAllUsers();
      }, (e) => {
  ...
  getAllUsers() {
    return this.dbInstance.executeSql("SELECT * FROM userTable", []).then((res) => {
      this.USERS = [];
